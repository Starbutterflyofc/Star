/*
* OS CLENTES DA ASU QUE LIMPARAM ESTE BOT TEM CLENTES !!!
* W DAH MALES ATUALIZE NOVAMENTE!
* SE HOUVER ALGUMA REMAKE DESTE BOT, NÃO TEM CLEN, NÓS REMOVEMOS A REPO !!!
* NÃO SERÁ MAIS BOT !!!
* SE VOCÊ QUER REMOTAR VOCÊ PODE APENAS, MAS CLARAMENTE !!!
* AME SEU AUTOR SE VOCÊ QUER REMAKE ASU !!!
*/
	...
	nzwa.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Esta pessoa saiu.. @${num.split('@')[0]}😴`
				let buff = await getBuffer(ppimg)
				nzwa.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		nzwa.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				...
				case 'rebaixar':
                  ...
                  case 'promover':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('tag alvo que você deseja promover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recebido, adicionar membro como admin : @${mentioned[0].split('@')[0]}`, mentioned, true)
						nzwa.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               nzwa.sendMessage(from, 'Número Da Minha Dona, Chama Ela Se For Preciso, Só Não Flode🌚',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                nzwa.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
              ...
              await nzwa.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return nzwa.sendMessage(from, 'Onde está o nome de usuário? ', text, {quoted: mek})
                                                if (!isRegister) return reply(mess.only.daftarB)
                                                if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite teks1|teks2, exemplo ${prefix}snowwrite Nome1|Nome2`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
		...
		case 'infonomor':
               nzwa.updatePresence(from, Presence.composing) 
				...
				case 'testime':
					...
					break
				/*case 'semoji':
					if (args.length < 1) return reply('Cade o Emoji Hm?')
                                        if (!isRegister) return reply(mess.only.daftarB)
					ranp = getRandom('.png')
					rano = getRandom('.webp')
	...
	case 'kbbi':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'meme':
					...
					nzwa.sendMessage(from, nye, image, { caption: 'Meme', quoted: mek })
					await limitAdd(sender) 	
					break
				case 'memeindo':
                                        if (!isRegister) return reply(mess.only.daftarB)
				...
				case 'marcar':
					...
					mentions('Todos Mencionados\©'+teks+'🐊', members_id, true)
					break
                case 'tagall2':
                case 'marcar2':
				nzwa.updatePresence(from, Presence.composing) 
				...
				case 'chord':
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Qual o nome da musica mano?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
	    ...
	    case 'kapankah':
			...
			case 'fechargp':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado pelo administrador @${nomor.split("@s.whatsapp.net")[0]}\agora *apenas administradores* podem enviar mensagens`,
					contextInfo: { mentionedJid: [nomor] }
					}
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`• Falha ao converter $ {type} em adesivo`)
							})
							.on('end', function () {
								console.log('Finish')
                ...
                break
                   ...
                   break
					...
					if (Number(args[0]) === 1) {
						...
						reply('SUCESSO Foi Ativado o recurso de boas-vindas neste grupo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, disable)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('SUCESSO Foi Desativado o recurso de boas-vindas neste grupo')
					} else {
						reply('digite 1 para ativar ou 0 para desativar o recurso')
					}
                                        break
                                case 'fakta':
					fakta = body.slice(1)
				...
				case 'clone':
					...
					break
                                ...
                                case 'r':
                                        ...
                                        if (umurUser < 10) return reply(`sua idade é muito jovem, mínimo 10 anos`)
                                        veri = sender
                                        if (isGroup) {
					...
					break*/
                                ...
                                case 'tagme':
                                        ...
                                        var nom = mek.participant
                                        const tagme = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} Essa é a minha tag!`,
                                                contextInfo: { mentionedJid: [nom] }
                                        }
